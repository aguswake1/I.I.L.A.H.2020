...
Champion* create_champion_op(string name)
{
	Champion* c = new Champion(name,100,100,100);
	return c;
}

void show_stats(Champion* c)
{
	cout << c->name << " " << c->hp << " " << c->strength << " " << c->attack_speed << endl;
}

int main()
{

	Champion* op = create_champion_op("Agustin");
	show_stats(op);
	op->hp = 30;
	show_stats(op);
	delete op;
